    ...
    <section class="bg-zinc-50 pt-8 pb-14 lg:pt-10 lg:pb-16">
            <div class="mx-auto w-full max-w-7xl px-6 md:px-10 lg:px-12">
                <div class="mx-auto max-w-3xl text-center">
                    <p class="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">{{ $contact['contact_page']['header']['support'][$lang] ?? 'Support' }}</p>
                    <h1 class="mt-3 text-3xl font-semibold tracking-tight text-black sm:text-4xl">{{ $contact['contact_page']['header']['title'][$lang] ?? 'Contact NoraPadel' }}</h1>
                    <p class="mt-4 text-zinc-600">{{ $contact['contact_page']['header']['subtitle'][$lang] ?? 'Send your questions. We will respond as soon as possible during business hours.' }}</p>
                </div>
                ...
                <div class="mx-auto mt-12 grid max-w-5xl gap-8 lg:grid-cols-[1fr_1.35fr]">
                    <div class="rounded-2xl border border-black/10 bg-white p-6 shadow-sm">
                        <h2 class="text-lg font-semibold text-black">{{ $contact['contact_page']['info_section']['title'][$lang] ?? 'Contact Information' }}</h2>
                        <ul class="mt-5 space-y-3 text-sm text-zinc-600">
                            ...
                            <li><span class="font-medium text-black">{{ $contact['contact_page']['info_section']['address_label'][$lang] ?? 'Address' }}:</span> {{ config('branding.address', $contact['contact_page']['info_section']['address_detail'][$lang] ?? 'Citraland, Surabaya, East Java, Indonesia') }}</li>
                        </ul>
                    </div>
                    ...
                    <form method="POST" action="{{ route('contact.submit') }}" class="rounded-2xl border border-black/10 bg-white p-6 shadow-sm">
                        ...
                        <div class="grid gap-5 sm:grid-cols-2">
                            <div class="sm:col-span-1">
                                <label for="name" class="mb-2 block text-sm font-medium text-zinc-700">{{ $contact['contact_page']['form']['label_name'][$lang] ?? 'Name' }}</label>
                            ...
                            <div class="sm:col-span-2">
                                <label for="subject" class="mb-2 block text-sm font-medium text-zinc-700">{{ $contact['contact_page']['form']['label_subject'][$lang] ?? 'Subject' }}</label>
                            ...
                            <div class="sm:col-span-2">
                                <label for="message" class="mb-2 block text-sm font-medium text-zinc-700">{{ $contact['contact_page']['form']['label_message'][$lang] ?? 'Message' }}</label>
                        ...
                        <button type="submit" class="mt-6 inline-flex rounded-full bg-black px-5 py-2.5 text-sm font-medium text-white transition hover:bg-zinc-800">{{ $contact['contact_page']['form']['btn_send'][$lang] ?? 'Send Message' }}</button>
                    </form>
                </div>
            </div>
        </section>
